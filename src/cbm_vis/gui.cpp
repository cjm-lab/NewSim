...
static void load_file(GtkWidget *widget, Control *control,
                      void (Control::*on_file_load_func)(std::string),
                      std::string err_msg) {
  std::string in_file_std_str = "";
  GtkWidget *dialog = gtk_file_chooser_dialog_new(
      "Open File", NULL, /* no parent window is fine for now */
...
static void save_file(GtkWidget *widget, Control *control,
                      void (Control::*on_file_save_func)(std::string),
                      std::string err_msg, const char *default_out_file) {
  std::string out_file_std_str = "";
  GtkWidget *dialog = gtk_file_chooser_dialog_new(
      "Save File", NULL, /* no parent window is fine for now */
      GTK_FILE_CHOOSER_ACTION_SAVE, "Cancel", GTK_RESPONSE_CANCEL, "Save",
      GTK_RESPONSE_ACCEPT, NULL);
...
static void save_file(GtkWidget *widget, Control *control,
                      std::function<void(std::string)> *save_func,
                      std::string err_msg, const char *default_out_file) {
  std::string out_file_std_str = "";
  GtkWidget *dialog = gtk_file_chooser_dialog_new(
      "Save File", NULL, /* no parent window is fine for now */
...
static void on_load_session_file(GtkWidget *widget, Control *control) {
  load_file(widget, control, &Control::initialize_session,
            "[ERROR]: Could not open session file.");
}

static void on_load_sim_file(GtkWidget *widget, Control *control) {
  // terribly stupid design atm. act params initialized when session is
  // initialized, so user needs to load session file before initializing
  // simulation. FIX IT
  if (!control->trials_data_initialized) {
    std::cout << "[ERROR]: Due to shitty design, you need to load a session "
                 "file before loading in a simulation file...\n";
    return;
  }
  load_file(widget, control, &Control::init_sim,
...
static void on_load_pfpc_weights(GtkWidget *widget, Control *control) {
  load_file(widget, control, &Control::load_pfpc_weights_from_file,
            "[ERROR]: Could not open pf-pc weights file.");
}

static void on_load_mfdcn_weights(GtkWidget *widget, Control *control) {
  load_file(widget, control, &Control::load_mfdcn_weights_from_file,
            "[ERROR]: Could not open mf-dcn weights file.");
}

static bool is_valid_dir_name(const char *in_str) {
  int len = snprintf(NULL, 0, "%s", in_str);
  if (len == 0 || len > 255)
    return false; // str length must be in the range (0,255]
  if (len == 1 && in_str[0] == '/')
    return false; // str cannot be just the path separator character
  return true;
}
...
char *create_dir_name_from(const char *base_path, const char *base_name) {
  ...
  if (base_path[base_path_len - 1] !=
      '/') // add in extra path separator token if not present in base_path
    strcat(full_path, "/");
...
static int create_dir_from(const char *base_path, const char *base_name,
...
static void create_dir_seq_on_valid_name(GtkWidget *parent_win,
  ...
  char *status_str;
  int status_str_len;
  if (create_dir_from(OUTPUT_DATA_PATH.c_str(), dir_base_name, overwrite) !=
      0) {
    status_str_len =
        snprintf(NULL, 0, "Could not create directory, '%s'", dir_base_name);
  ...
  GtkWidget *msg_dialog =
      gtk_message_dialog_new(GTK_WINDOW(parent_win), flags, GTK_MESSAGE_INFO,
                             GTK_BUTTONS_NONE, status_str);
...
static void on_create_dir(GtkWidget *widget, struct gui *gui) {
  ...
  GtkWidget *dialog = gtk_dialog_new_with_buttons(
      "Choose an Output Directory", GTK_WINDOW(gui->window), flags, "Cancel",
    ...
    case GTK_RESPONSE_OK:
      if (!is_valid_dir_name(gtk_entry_get_text(GTK_ENTRY(entry)))) {
        ...
        GtkWidget *msg_dialog = gtk_message_dialog_new(
            GTK_WINDOW(dialog), flags, GTK_MESSAGE_INFO, GTK_BUTTONS_NONE,
        ...
        GtkWidget *msg_dialog = gtk_message_dialog_new(
            GTK_WINDOW(dialog), flags, GTK_MESSAGE_INFO, GTK_BUTTONS_YES_NO,
            "This directory already exists: would you like to overwrite it?\n"
            "(Doing so will erase the current directory and all of its "
            "content.)");
        gtk_widget_show_all(msg_dialog);
        int overwrite = gtk_dialog_run(GTK_DIALOG(msg_dialog));
        switch (overwrite) {
        case GTK_RESPONSE_YES:
          // FIXME: reduce redundancy between this branch and the following else
          // branch
          create_dir_seq_on_valid_name(
              dialog, gtk_entry_get_text(GTK_ENTRY(entry)), true);
          if (gui->ctrl_ptr != NULL) {
            char *full_path = create_dir_name_from(
                OUTPUT_DATA_PATH.c_str(), gtk_entry_get_text(GTK_ENTRY(entry)));
            gui->ctrl_ptr->data_out_path = std::string(
                full_path); // data is safely copied to data_out_path
            gui->ctrl_ptr->data_out_base_name =
        ...
        create_dir_seq_on_valid_name(dialog,
                                     gtk_entry_get_text(GTK_ENTRY(entry)));
        if (gui->ctrl_ptr != NULL) {
          char *full_path = create_dir_name_from(
              OUTPUT_DATA_PATH.c_str(), gtk_entry_get_text(GTK_ENTRY(entry)));
          gui->ctrl_ptr->data_out_path =
              std::string(full_path); // data is safely copied to data_out_path
          gui->ctrl_ptr->data_out_base_name =
              std::string(gtk_entry_get_text(GTK_ENTRY(entry)));
          gui->ctrl_ptr->data_out_dir_created = true;
          free(full_path); // free current allocated memory
...
// FIXME: below is a stop-gap solution. should be more careful with destroy
// callback
gboolean firing_rates_win_visible(struct gui *gui) {
  return gui->frw.window != NULL && GTK_IS_WIDGET(gui->frw.window) &&
...
gboolean update_fr_labels(struct gui *gui) {
  ...
  FOREACH(gui->frw.cell_labels, clp) {
    ...
    FOREACH(*clp, rp) {
      if (fr_index > 0) {
        int len = snprintf(NULL, 0, "%.2f",
                           ((float *)(&fr[cell_index]))[fr_index - 1]);
...
static void on_firing_rates_window(GtkWidget *widget, struct gui *gui) {
  ...
  // g_signal_connect(gui->frw.window, "destroy",
  ...
  FOREACH(gui->frw.headers, hp) {
    ...
    gtk_grid_attach(GTK_GRID(gui->frw.grid), hp->label, hp->col, hp->row, 1, 1);
  }
  ...
  FOREACH(gui->frw.cell_labels, clp) {
    FOREACH(*clp, rp) {
      ...
      gtk_grid_attach(GTK_GRID(gui->frw.grid), rp->label, rp->col, rp->row, 1,
                      1);
    }
  }
...
static void on_tuning_window(GtkWidget *widget, struct gui *gui) {
  struct tuning_window tw = {
      ...
      .tuning_buttons = {
          {gtk_adjustment_new(gIncDirectMFtoGR, 0.0, 1.0, 0.0001, 0.1, 0.0),
           ...
           4,
           {NULL, "MF-GR", 0, 0},
           {"activate", G_CALLBACK(on_update_weight), &gIncDirectMFtoGR,
            false}},
          {gtk_adjustment_new(mfgoW, 0.0, 1.0, 0.0001, 0.1, 0.0),
           ...
           4,
           {NULL, "MF-GO", 0, 1},
           {"activate", G_CALLBACK(on_update_weight), &mfgoW, false}},
          {gtk_adjustment_new(grgoW, 0.0, 1.0, 0.0001, 0.1, 0.0),
           ...
           4,
           {NULL, "GR-GO", 0, 2},
           {"activate", G_CALLBACK(on_update_weight), &grgoW, false}},
          {gtk_adjustment_new(gogrW, 0.0, 1.0, 0.01, 0.1, 0.0),
           ...
           2,
           {NULL, "GO-GR", 0, 3},
           {"activate", G_CALLBACK(on_update_weight), &gogrW, false}},
          {gtk_adjustment_new(gogoW, 0.0, 1.0, 0.01, 0.1, 0.0),
           ...
           2,
           {NULL, "GO-GO", 2, 0},
           {"activate", G_CALLBACK(on_update_weight), &gogoW, false}},
          {gtk_adjustment_new(gIncGRtoPC, 0.0, 1.0, 0.000001, 0.1, 0.0),
           ...
           6,
           {NULL, "GR-PC", 2, 1},
           {"activate", G_CALLBACK(on_update_weight), &gIncGRtoPC, false}},
          {gtk_adjustment_new(gIncGRtoSC, 0.0, 1.0, 0.001, 0.1, 0.0),
           ...
           3,
           {NULL, "GR-SC", 2, 2},
           {"activate", G_CALLBACK(on_update_weight), &gIncGRtoSC, false}},
          {gtk_adjustment_new(gIncGRtoBC, 0.0, 1.0, 0.001, 0.1, 0.0),
           ...
           3,
           {NULL, "GR-BC", 2, 3},
           {"activate", G_CALLBACK(on_update_weight), &gIncGRtoBC, false}},
          {gtk_adjustment_new(gIncSCtoPC, 0.0, 1.0, 0.00001, 0.1, 0.0),
           ...
           5,
           {NULL, "SC-PC", 4, 0},
           {"activate", G_CALLBACK(on_update_weight), &gIncSCtoPC, false}},
          {gtk_adjustment_new(gIncBCtoPC, 0.0, 1.0, 0.00001, 0.1, 0.0),
           ...
           5,
           {NULL, "BC-PC", 4, 1},
           {"activate", G_CALLBACK(on_update_weight), &gIncBCtoPC, false}},
          {gtk_adjustment_new(gIncPCtoBC, 0.0, 1.0, 0.01, 0.1, 0.0),
           ...
           2,
           {NULL, "PC-BC", 4, 2},
           {"activate", G_CALLBACK(on_update_weight), &gIncPCtoBC, false}},
          {gtk_adjustment_new(gIncAvgPCtoNC, 0.0, 1.0, 0.001, 0.1, 0.0),
           ...
           3,
           {NULL, "PC-DCN", 4, 3},
           {"activate", G_CALLBACK(on_update_weight), &gIncAvgPCtoNC, false}},
          {gtk_adjustment_new(gAMPAIncMFtoNC, 0.0, 1.0, 0.001, 0.1, 0.0),
           ...
           3,
           {NULL, "MF-DCN", 6, 0},
           {"activate", G_CALLBACK(on_update_weight), &gAMPAIncMFtoNC, false}},
          {gtk_adjustment_new(gIncNCtoIO, 0.0, 1.0, 0.0001, 0.1, 0.0),
           ...
           4,
           {NULL, "DCN-IO", 6, 1},
           {"activate", G_CALLBACK(on_update_weight), &gIncNCtoIO, false}}}};
  ...
  FOREACH(tw.tuning_buttons, b) {
    ...
    gtk_grid_attach(GTK_GRID(tw.grid), b->label.label, b->label.col,
                    b->label.row, 1, 1);
    ...
    g_signal_connect(b->widget, b->signal.signal, b->signal.handler,
                     b->signal.data);
  }
...
static void draw_raster(GtkWidget *drawing_area, cairo_t *cr, uint32_t trial,
                        uint32_t num_cells, uint32_t num_row,
                        uint8_t **raster_data) {
...
static void draw_spatial_activity(GtkWidget *drawing_area, cairo_t *cr,
                                  Control *control) {
  /*
   * the plan:
   *     for cell coordinates x and y (in the square: [0, num_cell) x [0,
   * num_cell)), if cell at (x, y) fires at time step t_i, draw a dot with
   * alpha 1. for each time step after a spike, decrease alpha by a fixed
   * amount.
   *
   *     for now, call either g_threads_add_idle or directly emit signal to
   * re-draw every time step during a run. this allows for the view to animate
   * at a reasonable pace (e.g. 60 fps) when it is up, and then run the
   *     simulation at full speed when it is not. Useful for when you want to
   * check network activity after a certain number of training or forgetting
   * trials
   *
   */
}

static void draw_gr_raster(GtkWidget *drawing_area, cairo_t *cr,
                           Control *control) {
  /* 4096 gr raster sample size */
  draw_raster(drawing_area, cr, 0, 4096, control->msMeasure,
              control->rasters[GR]);
}

static void draw_go_raster(GtkWidget *drawing_area, cairo_t *cr,
                           Control *control) {
  draw_raster(drawing_area, cr, control->trial, num_go, control->msMeasure,
              control->rasters[GO]);
}

/* weights plot */
static void draw_pf_pc_plot(GtkWidget *drawing_area, cairo_t *cr,
                            Control *control) {
  ...
  float pfpc_w_to_pixel_scale_x =
      da.width / (float)4096; /* gr sample size is 4096 */
...
static void draw_pc_plot(GtkWidget *drawing_area, cairo_t *cr,
                         Control *control) {
  // background color setup
  cairo_set_source_rgb(cr, 0, 0, 0);
  cairo_paint(cr);
  ...
  cairo_rectangle(cr, control->msPreCS, -17, control->td.cs_lens[0],
                  9.5 * len_scale_y);
  cairo_fill(cr);
  ...
  for (uint32_t i = 0; i < control->msMeasure; i++) {
    ...
    for (int j = 0; j < num_pc; j++) {
      float vm_ij = control->pc_vm_raster[i][j] +
                    alternator * 0.2 * ceil(j / 2.0) * len_scale_y;
  ...
  for (int i = 0; i < control->msMeasure; i++) {
    ...
    for (int j = 0; j < num_nc; j++) {
      float vm_ij = nc_scale * control->nc_vm_raster[i][j] + nc_offset +
                    alternator * 0.2 * ceil(j / 2.0) * len_scale_y;
      cairo_rectangle(cr, i, vm_ij, 1.0, 0.05);
  ...
  for (int i = 0; i < control->msMeasure; i++) {
    ...
    for (int j = 0; j < num_io; j++) {
      float vm_ij = io_scale * control->io_vm_raster[i][j] + io_offset +
                    alternator * 0.2 * ceil(j / 2.0) * len_scale_y;
      cairo_rectangle(cr, i, vm_ij, 1.0, 0.05);
...
static void set_gui_window_attribs(struct gui *gui) {
  ...
  gtk_window_set_default_size(GTK_WINDOW(gui->window), MAIN_WINDOW_WIDTH,
                              MAIN_WINDOW_HEIGHT);
...
static void set_gui_dcn_plast_button_attribs(struct gui *gui) {
  ...
  gtk_grid_attach(GTK_GRID(gui->grid), gui->dcn_plast_button.widget,
                  gui->dcn_plast_button.col, gui->dcn_plast_button.row, 1, 1);
  g_signal_connect(
      gui->dcn_plast_button.widget, gui->dcn_plast_button.signal.signal,
      gui->dcn_plast_button.signal.handler, gui->dcn_plast_button.signal.data);
}

static void set_gui_radio_button_attribs(struct gui *gui) {
  gtk_grid_attach(GTK_GRID(gui->grid), gui->plast_radio_label, 0, 2, 1,
                  1); /* hard coded col, row nums for now */
  ...
  FOREACH(gui->plasticity_radios, r) {
    ...
    gtk_radio_button_join_group(GTK_RADIO_BUTTON(r->widget),
                                GTK_RADIO_BUTTON(group));
    if (r->label == "Graded")
      group = r->widget; /* hack to ensure first radio group NULL, rest are
                            first radio */
  ...
  FOREACH_NELEM(menu->menu_items, menu->num_menu_items, mi) {
             ...
             */
    {
      if (mi->label != "") /* by extension, every menu_item has a label */
      {
        ...
        gtk_menu_item_set_submenu(GTK_MENU_ITEM(mi->menu_item),
                                  mi->sub_menu.menu);
      }
      if (mi->signal.signal) /* little hack for checking whether mi->signal was
                                non empty initialized */
      {
        if (mi->signal.swapped) {
          g_signal_connect_swapped(mi->menu_item, mi->signal.signal,
                                   mi->signal.handler, mi->signal.data);
        } else {
          g_signal_connect(mi->menu_item, mi->signal.signal, mi->signal.handler,
                           mi->signal.data);
        }
      }
...
static void set_gui_menu_item_helper(struct menu_item *menu_item) {
  if (!menu_item->sub_menu.menu)
    return; // little hack for checking whether sub_menu was empty initialized
...
static void free_gui_menu_helper(struct menu *menu) {
  FOREACH_NELEM(menu->menu_items, menu->num_menu_items, mi) {
    if (mi->sub_menu.menu)
      free_gui_menu_helper(&mi->sub_menu);
  }
...
int gui_init_and_run(int *argc, char ***argv, Control *control) {
  ...
  save_data save_data[NUM_SAVE_OPTS] = {
      {control, MF_RAST}, {control, GR_RAST}, {control, GO_RAST},
      {control, BC_RAST}, {control, SC_RAST}, {control, PC_RAST},
      {control, IO_RAST}, {control, NC_RAST}, {control, MF_PSTH},
      {control, GR_PSTH}, {control, GO_PSTH}, {control, BC_PSTH},
      {control, SC_PSTH}, {control, PC_PSTH}, {control, IO_PSTH},
      {control, NC_PSTH}, {control, PFPC},    {control, MFNC},
      {control, SIM}};
  ...
  struct gui gui = {
      ...
      .dcn_plast_button = {"DCN Plasticity",
                           gtk_check_button_new(),
                           1,
                           4,
                           {"clicked", G_CALLBACK(on_dcn_plast), control,
                            false}},
      .plast_radio_label = gtk_label_new("Plasticity"),
      .plasticity_radios = {{"Graded",
                             gtk_radio_button_new(NULL),
                             0,
                             3,
                            ...
                            {"Off",
                             ...
                             0,
                             7,
                             {"toggled", G_CALLBACK(on_radio), control,
                              false}}},
      .menu_bar =
          {.menu = gtk_menu_bar_new(),
           .num_menu_items = NUM_SUB_MENU_ITEMS,
           .menu_items =
               new menu_item[NUM_SUB_MENU_ITEMS]{
                   {"File",
                    gtk_menu_item_new(),
                    {},
                    {gtk_menu_new(), NUM_FILE_MENU_ITEMS,
                     new menu_item[NUM_FILE_MENU_ITEMS]{
                         {"Load...",
                          gtk_menu_item_new(),
                          {},
                          {gtk_menu_new(), NUM_FILE_SUB_MENU_ITEMS,
                           new menu_item[NUM_FILE_SUB_MENU_ITEMS]{
                               {"Session File",
                                gtk_menu_item_new(),
                                {"activate", G_CALLBACK(on_load_session_file),
                                 control, false},
                                {}},
                               {"Simulation File",
                                gtk_menu_item_new(),
                                {"activate", G_CALLBACK(on_load_sim_file),
                                 control, false},
                                {}}}}},
                         {"Save Sim",
                          gtk_menu_item_new(),
                          {"activate", G_CALLBACK(on_save_file),
                           &save_data[SIM], false},
                          {}},
                         {"Create Output Dir",
                          gtk_menu_item_new(),
                   ...
                   {"Weights",
                    gtk_menu_item_new(),
                    {},
                    {gtk_menu_new(), NUM_WEIGHTS_MENU_ITEMS,
                     new menu_item[NUM_WEIGHTS_MENU_ITEMS]{
                         {"Save PF-PC",
                          gtk_menu_item_new(),
                          {"activate", G_CALLBACK(on_save_file),
                           &save_data[PFPC], false},
                          {}},
                         {"Load PF-PC",
                          gtk_menu_item_new(),
                          {"activate", G_CALLBACK(on_load_pfpc_weights),
                           control, false},
                          {}},
                         {"Save MF-DN",
                          gtk_menu_item_new(),
                          {"activate", G_CALLBACK(on_save_file),
                           &save_data[MFNC], false},
                          {}},
                         {"Load MF-DN",
                          gtk_menu_item_new(),
                          {"activate", G_CALLBACK(on_load_mfdcn_weights),
                           control, false},
                          {}}}}},
                   {"Raster",
                    gtk_menu_item_new(),
                    {},
                    {gtk_menu_new(), NUM_RASTER_MENU_ITEMS,
                     new menu_item[NUM_RASTER_MENU_ITEMS]{
                         {"Save GR",
                          gtk_menu_item_new(),
                          {"activate", G_CALLBACK(on_save_file),
                           &save_data[GR_RAST], false},
                          {}},
                         {"Save GO",
                          gtk_menu_item_new(),
                          {"activate", G_CALLBACK(on_save_file),
                           &save_data[GO_RAST], false},
                          {}},
                         {"Save PC",
                          gtk_menu_item_new(),
                          {"activate", G_CALLBACK(on_save_file),
                           &save_data[PC_RAST], false},
                          {}},
                         {"Save DCN",
                          gtk_menu_item_new(),
                          {"activate", G_CALLBACK(on_save_file),
                           &save_data[NC_RAST], false},
                          {}},
                         {"Save IO",
                          gtk_menu_item_new(),
                          {"activate", G_CALLBACK(on_save_file),
                           &save_data[IO_RAST], false},
                          {}},
                         {"Save BC",
                          gtk_menu_item_new(),
                          {"activate", G_CALLBACK(on_save_file),
                           &save_data[BC_RAST], false},
                          {}},
                         {"Save SC",
                          gtk_menu_item_new(),
                          {"activate", G_CALLBACK(on_save_file),
                           &save_data[SC_RAST], false},
                          {}},
                         {"Save MF",
                          gtk_menu_item_new(),
                          {"activate", G_CALLBACK(on_save_file),
                           &save_data[MF_RAST], false},
                          {}},
                     }}},
                   {"PSTH",
                    gtk_menu_item_new(),
                    ...
                    {gtk_menu_new(), NUM_RASTER_MENU_ITEMS,
                     new menu_item[NUM_RASTER_MENU_ITEMS]{
                         {"Save GR",
                          gtk_menu_item_new(),
                          {"activate", G_CALLBACK(on_save_file),
                           &save_data[GR_PSTH], false},
                          {}},
                         {"Save GO",
                          gtk_menu_item_new(),
                          {"activate", G_CALLBACK(on_save_file),
                           &save_data[GO_PSTH], false},
                          {}},
                         {"Save PC",
                          gtk_menu_item_new(),
                          {"activate", G_CALLBACK(on_save_file),
                           &save_data[PC_PSTH], false},
                          {}},
                         {"Save DCN",
                          gtk_menu_item_new(),
                          {"activate", G_CALLBACK(on_save_file),
                           &save_data[NC_PSTH], false},
                          {}},
                         {"Save IO",
                          gtk_menu_item_new(),
                          {"activate", G_CALLBACK(on_save_file),
                           &save_data[IO_PSTH], false},
                          {}},
                         {"Save BC",
                          gtk_menu_item_new(),
                          {"activate", G_CALLBACK(on_save_file),
                           &save_data[BC_PSTH], false},
                          {}},
                         {"Save SC",
                          gtk_menu_item_new(),
                          {"activate", G_CALLBACK(on_save_file),
                           &save_data[SC_PSTH], false},
                          {}},
                         {"Save MF",
                          gtk_menu_item_new(),
                          {"activate", G_CALLBACK(on_save_file),
                           &save_data[MF_PSTH], false},
                          {}},
                     }}},
                   {"Analysis",
                    gtk_menu_item_new(),
                    {},
                    {gtk_menu_new(), NUM_ANALYSIS_MENU_ITEMS,
                     new menu_item[NUM_ANALYSIS_MENU_ITEMS]{
                         {"Firing Rates",
                          gtk_menu_item_new(),
                   ...
                   {"Tuning",
                    gtk_menu_item_new(),
                    {},
                    {gtk_menu_new(), NUM_TUNING_MENU_ITEMS,
                     new menu_item[NUM_TUNING_MENU_ITEMS]{
                         {"Tuning",
                          gtk_menu_item_new(),
                          {"activate", G_CALLBACK(on_tuning_window), &gui,
                           false},
                          {}},
                     }}}}},
      .frw = {.window = NULL,
  ...
  // again."); gtk_box_pack_start(GTK_BOX(msg_area), check_btn_label, FALSE,
  // TRUE, 0); gtk_box_pack_start(GTK_BOX(msg_area), dialog_check_btn, FALSE,
  // TRUE, 0);
